...
# 2026-09-14 依使用者要求：0.85 → 0.60 → 0.70 → 0.85 → None（24h 94 筆 -$2,025，37 筆停損平均 -$58，改回不停損）。
LATE_FAVORITE_STOP_LOSS_PRICE  = None
...
for _asset in ASSETS:
    ...
    if _asset["id"] == "btc":
        ...
        AB_VARIANTS.append({
            ...
            "favoriteStopLossPrice": 0.60,   # 2026-09-16 依使用者要求 0.60 → 0.85 → 0.60
            "favoriteTakeProfitPrice": None,
            "favoriteStableSeconds": 0.0,
        })
        AB_VARIANTS.append({
...
def _favorite_family_for_asset(asset: dict) -> list[dict]:
    ...
    fam = [
        ...
        dict(common, id=f"{aid}-last60-098-hold", label=f"{label} 最後 {60 * k:.0f} 秒買 ≥0.98 抱到結算（穩定 10s、翻面偵測）",
             ...
             simOnly=False),   # 2026-09-18 開放實盤選用（實盤② 改用 ETH）
...
_SIM_DEFAULT_DISABLED = (
    ...
    # 2026-09-19：btc-last60-098-hold（穩定 10s＋翻面偵測）重新啟用，作為實盤① 無過濾版的對照。
    "btc-follow-taker,btc-price-triggered-favorite,"
