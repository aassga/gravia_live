...
`btc-15m` 改為單一、獨立記帳的 `BTC 15m 暴跌後補腿（3s/-15%）` 紙上策略；使用新的 `btc-15m-dump-then-hedge` variant id，因此不會混入先前兩個 15 分鐘策略的績效：

- 僅在每個 15 分鐘窗口開始後前 `120s` 尋找 Leg 1；比較 Polymarket WebSocket 同一 outcome token 的 best ask 與約 `3s` 前價格，跌幅達 `15%` 才買入下跌的一腿。
- 每次固定模擬 `5 shares`，必須由最新完整 WebSocket 訂單簿一次吃滿；成交模型保留既有的完整深度 VWAP、`3 bps` 不利滑價、一格 tick 限價緩衝、最低股數與 taker fee。
- Leg 1 成交後不再買同方向，只等待相反 outcome 的 `5 shares`。只有兩腿保守決策價合計不超過 `$0.95`，且扣除兩腿動態 taker fee 後仍至少保留 `1¢/share` 淨利，才模擬補腿。
- Leg 2 沒有 120 秒限制，可以等到窗口結束；始終補不到時保留單腿到正式結算，不假裝平倉成功，以便如實量出這個策略的最大風險。
- 完成一組後可在同一市場繼續找下一組，但會清空舊的三秒價格歷史，必須出現新的完整跌幅訊號，不能用同一個瞬間重複成交。
- Dashboard 額外顯示訊號數、Leg 1 進場數、完整補腿數、未對沖結算數；每筆交易保留跌幅、參考價、等待補腿秒數、費用與費後損益。

這組策略只存在於模擬盤，不會改變或啟用任何真實下單流程。它刻意把 Leg 1 到 Leg 2 之間視為方向性曝險，而不是把 `ask1 + ask2 ≤ 0.95` 誤稱為第一腿成交當下就已鎖利。

方向組的下注股數直接以單腿實際決策成本和費用反推，不再錯用兩腿預算公式。這些門檻是保守的紙上驗證起點，必須累積足夠樣本、分別檢查勝率、期望值、最大回撤與訊號拒絕原因後，才適合討論實盤。

## 真實自動下單

`polymarket_live_strategy.py` 已同步紙上模擬的深度 VWAP、最差限價判斷、滑點、資金預留與淨鎖利規則。直接配對進場會先預熱新市場 token 的建單 metadata 與 V2 動態費率，再由預熱過的專用執行緒把 Up／Down 兩筆 FOK 簽名；簽名完成後會立即重讀最新 WebSocket 訂單簿，確認原本簽好的兩個限價仍能完整覆蓋最新深度且鎖利條件仍成立，通過後不先寫診斷或 log，立即由獨立 POST 執行緒以同一次 `POST /orders` batch 送出。這能排除在本機簽名期間已消失的機會、避免共用 thread pool 排隊，並減少兩個獨立 request 的到達時間差。成交後會優先從成交紀錄回填真實均價；暫時查不到時則以送出的保守限價記帳。Batch 仍不保證兩筆原子成交，因此其中一腿失敗時仍會先補該腿，最後才嘗試緊急賣回已成交腿。
...
- `POLY_PAIR_MIN_DEPTH_MULTIPLIER=1.3`：每腿在實際送出限價內的可成交深度，至少要達下單股數的 1.3 倍；例如每腿送 5 股時，兩邊都必須各有至少 6.5 股可執行深度。
