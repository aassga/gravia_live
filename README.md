...
- `BTC 歷史混合（鎖利→Binance T-10s）` 是另一個從零記帳的純模擬組，用來重現舊版高交易數的真實流程：整個窗口先嘗試兩腿直接鎖利，沒有合格配對才在 T-3～10 秒使用同一套 Binance 方向訊號。它保留現行 WebSocket 報價同步、完整深度、滑價、費用與最低淨利防護，且不會被實盤策略引用。
...
- `BTC 15m 深度確認兩腿鎖利`：兩腿保守決策價合計最多 `$0.95`，扣費後至少保留 `1¢/股`；兩腿限價內深度都要達下注量 `2x`，而且機會需連續存在 `0.20s`。每次最多使用資產 `10%`、上限 `$20`，保留 `$10` 現金。
- `BTC 15m Chainlink 自適應方向性`：只在 T-12～30 秒評估，必須有精確窗口開盤 Chainlink TWAP 與 2.5 秒內的最新值。方向偏移至少 `0.04%`，並用最近 5 分鐘、不重疊 10 秒區間的 Chainlink TWAP 波動按剩餘時間估算維持方向機率；估計機率至少 `78%`、Polymarket 正規化 mid 至少 `55%`、spread 最多 `5¢`、保守買價最多 `$0.88`，且扣費後模型 edge 至少 `3¢/股`。訊號及全部風控需同方向連續成立 `1.5s`，限價內深度至少 `2x`。單腿每次只使用資產 `5%`、最多 `$10`，並保留 `$10` 現金。
